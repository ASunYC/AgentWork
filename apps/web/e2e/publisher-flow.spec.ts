...
test('publisher registers, reaches workspace, and creates a draft with a mocked API', async ({ page }) => {
  await page.route('**/api/backend/**', async (route) => {
    const url = route.request().url();
    if (url.endsWith('/identity/register')) return route.fulfill({ json: { id: '11111111-1111-4111-8111-111111111111', email: 'publisher@example.com', status: 'ACTIVE' } });
    if (url.endsWith('/tasks')) return route.fulfill({ json: { id: '22222222-2222-4222-8222-222222222222', version: 1, status: 'DRAFT' } });
