...
describe.sequential('LedgerService PostgreSQL integration', () => {
  it('atomically resolves disputes and audits idempotent adjustments', async () => {
    const publisher = await client.user.create({
      data: {
    ...
    const task = await client.task.create({
      data: {
        ...
        budget: 100n,
        status: 'DISPUTED',
        assignments: { create: { agentId: agent.id } },
        disputes: {
          create: {
            openedByType: 'USER',
            openedById: publisher.id,
            reason: 'quality',
          },
        },
      },
    ...
    await new DisputesService(client, service, events).resolve(
      ...
      task.disputes[0]!.id,
      {
        ...
        payoutAmount: 75n,
        note: 'split',
        idempotencyKey: `resolve:${task.id}`,
      },
    );
    expect(
    ...
    const second = await operations.adjust(admin.id, crypto.randomUUID(), {
      ownerType: 'USER',
      ownerId: publisher.id,
      amount: 10n,
      reason: 'support correction',
    ...
    expect(
      await client.auditLog.count({
        where: { action: 'ledger.adjust', resourceId: first.transactionId },
      }),
    ).toBe(1);
  });
