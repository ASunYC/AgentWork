...
export const deriveEndpointSecret = (
  master: string,
  endpointId: string,
  version: number,
) =>
...
export class WebhookDeliveryEngine {
  ...
  async materialize() {
    ...
    for (const event of events) {
      const endpoints = await this.db.agentEndpoint.findMany({
        where: { agentId: event.subjectId, verifiedAt: { not: null } },
        select: { id: true },
      });
      for (const endpoint of endpoints)
        await this.db.webhookDelivery.upsert({
          ...
          create: { eventId: event.id, endpointId: endpoint.id, attempt: 1 },
          update: {},
        });
    }
  }
  ...
  async deliver(delivery: Delivery) {
    ...
    const secret = deriveEndpointSecret(
      this.masterSecret,
      delivery.endpoint.id,
      delivery.endpoint.secretVersion,
    );
    try {
      const response = await this.fetcher(delivery.endpoint.webhookUrl, {
        ...
        headers: {
          ...
          'x-agentwork-timestamp': timestamp,
          'x-agentwork-signature': `sha256=${signWebhook(secret, timestamp, body)}`,
          'x-agentwork-delivery': delivery.id,
        },
      });
      const responseBody = (await response.text()).slice(0, 4096);
      if (!response.ok)
        return this.fail(delivery, response.status, responseBody);
